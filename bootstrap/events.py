...
class Subject:
    ...
    def check_conditions(self, event, watched_event):
    ...
    def invoke_event_callbacks(self, event):
        if event.type in self._watched_events.keys():
            for watched_event in self._watched_events[event.type]:
                if hasattr(watched_event, 'conditions') and not self.check_conditions(event, watched_event):
                    continue
...
class EventToWatch:
    def __init__(self, type, callback, name=None, conditions=None):
        self.name = name
        self.type = type
...
class Events:
    ...
    def on(self, type, callback, name=None, conditions=None):
        self.subject.register_event(EventToWatch(type, callback, name, conditions))
